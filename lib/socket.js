...
setInterval(() => { store?.writeToFile('./session/store.json') }, 10000)

async function startSock() { const { state, saveCreds } = await useMultiFileAuthState('./session') const { version, isLatest } = await fetchLatestBaileysVersion()
